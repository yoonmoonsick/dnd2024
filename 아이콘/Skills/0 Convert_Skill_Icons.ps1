...
New-Item -ItemType Directory -Force -Path "3_Atalas_64" | Out-Null
...
foreach ($file in $files) {
    ...
    & cmd /c "magick `spell_bg.DDS` -resize 144x144 ( `"$fullPath`" -resize 144x144 ) -gravity center -composite `3_Atalas_64\$filename.png`"
    # 작업 3: PNG를 DDS로 변환 (BC7 압축)
    & cmd /c "magick `3_Atalas_64\$filename.png` -define dds:compression=BC7 `3_Atalas_64\$filename.DDS`"
}
